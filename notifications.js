 ...
 * امکانات:
 *
 * 1. دریافت اعلان‌های کاربر
 * 2. دریافت اعلان‌های خوانده‌نشده
 * 3. شمارش اعلان‌های خوانده‌نشده
 * 4. علامت‌گذاری یک اعلان به‌عنوان خوانده‌شده
 * 5. علامت‌گذاری همه اعلان‌ها به‌عنوان خوانده‌شده
 * 6. حذف اعلان
 * 7. حذف همه اعلان‌ها
 * 8. ساخت اعلان از سمت سیستم
 * 9. نمایش اعلان داخل UI
 * 10. پنل اعلان‌ها
 * 11. Badge تعداد اعلان‌های خوانده‌نشده
 * 12. Realtime Notifications
 * 13. هماهنگی با Auth
 * 14. هماهنگی با Profile
 * 15. هماهنگی با بازی
 * 16. Toast Notification
 * 17. Browser Notification در صورت اجازه کاربر
 * 18. مدیریت خطا
 * 19. Cache داخلی
 * 20. Event System
 * 21. API عمومی window.hokmNotifications
 *
 * Database table:
 *
        ...
        listeners.forEach(
            callback => {
                ...
                try {
                    ...
                    console.error(
                        `خطا در Notification Event: ${eventName}`,
                        error
                    );
...
async function loadNotifications(
    ...
    const client =
        getNotificationsSupabaseClient();
    ...
    try {
        ...
        notificationState.lastLoadedAt =
            new Date();
...
async function markAllAsRead() {
    ...
    try {
        ...
        if (error) {
            ...
            console.error(
                "خطا در خوانده‌شدن همه اعلان‌ها:",
                error
            );


            notificationToast(
                "علامت‌گذاری اعلان‌ها انجام نشد.",
                "❌"
            );
...
/* ================================================================
   26. CREATE CURRENT USER NOTIFICATION
================================================================ */

async function notifyCurrentUser(
    title,
    message,
    type = "system"
) {
...
async function showBrowserNotification(
    ...
    try {
        ...
        console.warn(
            "نمایش Browser Notification ممکن نیست:",
            error
        );
...
/* ================================================================
   28. SHOW IN-GAME NOTIFICATION
================================================================ */

function showInGameNotification(
    notification
) {
...
function startNotificationsRealtime() {
    ...
    if (
        notificationState.realtimeStarted
    ) {
    ...
    try {
        ...
        notificationState.currentChannel =
            channel;
...
async function stopNotificationsRealtime() {
    ...
    try {
        ...
        console.warn(
            "خطا در توقف Notifications Realtime:",
            error
        );
        ...
        >
            ...
            <div class="hokm-notification-content">
                ...
                <div class="hokm-notification-header">
                    ...
                    <strong class="hokm-notification-title">
                        ${title}
                    </strong>
                    ...
                    ${
                        notification.is_read
                            ...
                            : `
                                <span
                                    class="hokm-notification-unread-dot"
                ...
                <p class="hokm-notification-message">
                    ${message}
                </p>

                <time
                    class="hokm-notification-time"
                    datetime="${escapeNotificationHTML(notification.created_at)}"
                    title="${fullDate}"
                >
                    ${relativeTime}
                </time>
            ...
            <div class="hokm-notification-actions">
                ...
                ${
                    notification.is_read
                        ...
                        : `
                            <button
                                type="button"
                                class="hokm-notification-action"
                                data-notification-read="${escapeNotificationHTML(notification.id)}"
                                title="خوانده شد"
                ...
                <button
                    type="button"
                    class="hokm-notification-action hokm-notification-delete"
                    data-notification-delete="${escapeNotificationHTML(notification.id)}"
                    title="حذف اعلان"
    ...
    if (
        ...
        target.innerHTML = `
            <div class="hokm-notifications-empty">
                ...
                <div class="hokm-notifications-empty-icon">
                    🔔
                </div>
                ...
                <p>
                    وقتی اتفاق مهمی در بازی رخ دهد،
                    اعلان آن اینجا نمایش داده می‌شود.
                </p>
...
/* ================================================================
   38. AUTH STATE HANDLER
================================================================ */

async function handleNotificationAuthChange(
    data
) {
...
async function initializeNotifications() {
    ...
    console.log(
        "Hokm Online Notifications initialized successfully."
    );
}
...
function bindNotificationEvents() {
    ...
    if (
        document.body.dataset.notificationsBound ===
        "true"
    ) {
...
async function requestNotificationPermission() {
    ...
    if (
        ...
        notificationToast(
            "مرورگر شما اعلان‌های سیستمی را پشتیبانی نمی‌کند.",
            "⚠️"
        );
...
function bindGameNotificationEvents() {

    /*
     * این بخش به‌صورت انعطاف‌پذیر با
     * سیستم Event بازی کار می‌کند.
     */


    if (
        window.hokmGame &&
        typeof window.hokmGame.on === "function"
    ) {
    ...
    if (
        ...
        window.hokmAuth.onSignOut(
            async () => {
                ...
                await handleNotificationAuthChange(
                    {
                        user: null
                    }
                );
...
async function startNotifications() {
    ...
    /*
     * ابتدا Eventهای Auth را متصل می‌کنیم.
     */
    ...
    bindGameNotificationEvents();


    /*
     * اگر Auth قبلاً آماده شده باشد،
     * مستقیم Notifications را اجرا می‌کنیم.
     */

    if (
        window.hokmAuth &&
        typeof window.hokmAuth.waitForAuth ===
        "function"
    ) {
